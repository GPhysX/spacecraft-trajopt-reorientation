...
syms m1 m2 m3 m4 M J Ih Jh x0 y0 t
...
equ_cm = simplify(m1*(rm1G)+M*(rMG)+m2*(rm2G)+m3*(rm3G)+m4*(rm4G));
...
Hm = simplify(m1*cross(rm1G,diff(rm1G,t))+m2*cross(rm2G,diff(rm2G,t))+m3*cross(rm3G,diff(rm3G,t))+m4*cross(rm4G,diff(rm4G,t)) ); %angular momentum of m about system c.m. in G
HM = simplify(J*diff(th,t)*Kh+M*cross(rMG,diff(rMG,t))); %angular momentum of M about system c.m. in G

syms x1 y1 x2 y2 x3 y3 x4 y4 th x1d y1d x2d y2d x3d y3d x4d y4d thetad
equ1=subs(simplify(Hm(3)+HM(3)),...
    {str2sym('x1(t)'),str2sym('y1(t)'),...
     str2sym('x2(t)'),str2sym('y2(t)'),str2sym('x3(t)'),str2sym('y3(t)'),...
     str2sym('x4(t)'),str2sym('y4(t)'),str2sym('th(t)'),...
     ...
     str2sym('diff(th(t), t)')},[x1,y1,x2,y2,x3,y3,x4,y4,th,x1d,y1d,x2d,y2d,x3d,y3d,x4d,y4d,thetad]);

disp('Equation for thetadot:');
collect(simplify(solve(equ1,thetad)),[x1d,y1d,x2d,y2d,x3d,y3d,x4d,y4d])
